...
<body>
    ...
    <section>
        ...
        <div id="board_box">
            ...
            <ul class="buttons">
                <li><button
                        onclick="location.href='board_text_good_action.php?num=<?= $num ?>&page=<?= $page ?>'">추천</button>
                </li>
                <li><button onclick="location.href='board_text_list.php?page=<?= $page ?>'">목록</button></li>
                <li><button
                        onclick="location.href='board_text_change.php?num=<?= $num ?>&page=<?= $page ?>'">수정</button>
                </li>
                <li><button
                        onclick="location.href='board_text_delete_action.php?num=<?= $num ?>&page=<?= $page ?>'">삭제</button>
                </li>
                <li><button onclick="location.href='board_text.php'">글쓰기</button></li>
            </ul>
            <ul id="reply">
                <form action="reply_action.php" class="reply" name="reply" method="POST">
                    <input type="hidden" name="num" value="<?= $num ?>">
                    <input type="hidden" name="board_type" value="text">
                    <input type="text" name="content" class="text" autocomplete=off />
                    <input type="submit" class="button" name="submit" value="댓글 작성" />
                </form>

                <?php
                $sql = "SELECT * FROM reply WHERE num=$num AND board_type='text' ORDER BY regist_day";
                $result = mysqli_query($conn, $sql);

...
if (Prototype.Browser.WebKit) {
  $A = Array.from = function (iterable) {
    if (!iterable) return [];
    if (
      !(typeof iterable == "function" && iterable == "[object NodeList]") &&
      iterable.toArray
    ) {
      return iterable.toArray();
    } else {
...
Ajax.PeriodicalUpdater.prototype = Object.extend(new Ajax.Base(), {
  ...
  updateComplete: function (request) {
    if (this.options.decay) {
      this.decay =
        request.responseText == this.lastText
...
Element.Methods = {
  ...
  toggleClassName: function (element, className) {
    ...
    Element.classNames(element)[
      element.hasClassName(className) ? "remove" : "add"
    ](className);
    return element;
  },
  ...
  compileMatcher: function () {
    ...
    while (e && le != e && /\S/.test(e)) {
      ...
      for (var i in ps) {
        ...
        if ((m = e.match(p))) {
          this.matcher.push(
            typeof c[i] == "function" ? c[i](m) : new Template(c[i]).evaluate(m)
          );
          e = e.replace(m[0], "");
          break;
        }
